...
class CurriculumApiIntegrationTest : IntegrationTestSupport() {
    @Test
    @DisplayName("활성 커리큘럼 6개를 displayOrder 순으로 반환한다")
    fun shouldReturnActiveCurriculumsOrderedByDisplayOrder() {
        ...
        expectApiSuccess(get("/curriculum", deviceId))
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.data[0].code").value("FRACTION_CALC"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data[0].name").value("분수의 계산"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.data[5].code").value("SCIENCE_STRATA_FOSSIL"))
    }
}
